...
def search():
    ...
    if ip:
        ...
        return render_template("index.html", data={"error": "IP address not provided"}, lang=lang, translations=TRANSLATIONS[lang])

@app.route("/export/<ip>")
def export_analysis(ip):
    ...
    try:
        ...
        return send_file(
            ...
            as_attachment=True,
            download_name=f'netscan_analysis_{ip}_{dt.now().strftime("%Y%m%d_%H%M%S")}.json'
        )
    except Exception as e:
        return jsonify({"error": str(e)}), 400
...
def bulk_analysis():
    if request.method == 'POST':
        ip_list = request.form['ip_list'].split('\n')
        results = analyze_multiple_ips(ip_list)
